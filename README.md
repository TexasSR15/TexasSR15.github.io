

    
    
      
     
    


    
    
 
      
 
 
    
      
     
 
         <!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <title>Word War Multiplayer</title>
  <style>
    /* Global Styles */
    body {
      background: #f0f0f0;
      font-family: Arial, sans-serif;
      margin: 0;
      padding: 20px;
      text-align: center;
      color: #333;
    }
    
    h2 {
      font-size: 2.5em;
      margin-bottom: 20px;
      color: #222;
    }
    
    p#instructions {
      font-size: 1.2em;
      margin: 20px auto;
      width: 80%;
      max-width: 600px;
      background: #fff;
      padding: 15px;
      border-radius: 8px;
      box-shadow: 0 2px 5px rgba(0,0,0,0.1);
    }
    
    /* Button Styling */
    button {
      background-color: #007bff;
      border: none;
      padding: 10px 20px;
      font-size: 1em;
      border-radius: 5px;
      color: #fff;
      cursor: pointer;
      margin: 10px;
      transition: background-color 0.3s ease;
    }
    
    button:hover {
      background-color: #0056b3;
    }
    
    /* Input Field Styling */
    input[type="text"] {
      padding: 10px;
      font-size: 1em;
      border: 2px solid #ddd;
      border-radius: 5px;
      margin: 10px;
      width: 200px;
      transition: border-color 0.3s ease;
    }
    
    input[type="text"]:focus {
      border-color: #007bff;
      outline: none;
    }
  </style>
</head>
<body>

<h2>Word War Multiplayer</h2>
<p id="instructions"></p>
<button id="button1">Start Game</button>

<script>
  /***********************
   * Multiplayer Settings *
   ***********************/
  // For demonstration, we assume two players.
  // In a real application, you would gather the names from your user system.
  let players = ["Alice", "Bob"];
  let currentPlayerIndex = 0; // Whose turn it is

  /***********************
   * Word War Game Code  *
   ***********************/
  let startingWord = "fire";   // Initial word
  let sWord = startingWord;    // Current word
  let button1 = document.querySelector("#button1");
  let instructions = document.querySelector("#instructions");
  let userInput2;      // For use in unSplit
  let splitButton;     // For use in unSplit
  let usedWords = [];
  usedWords.push(startingWord);

  // When the game starts, announce whose turn it is.
  button1.onclick = startGame;

  function startGame() {
    // Create an input field for the word
    let userInput = document.createElement("input");
    userInput.id = "userInput";
    userInput.type = "text";
    document.body.appendChild(userInput);

    // Announce the starting word and which player's turn it is
    instructions.innerText = players[currentPlayerIndex] + ", your current word is '" + startingWord + "'.";
    button1.innerText = "Submit";
    button1.onclick = valCheck;

    // Create the split button (for splitting moves)
    splitButton = document.createElement("button");
    splitButton.innerText = "Split";
    document.body.appendChild(splitButton);
    splitButton.onclick = split;
  }

  // Call this after a valid move to change turns
  function nextTurn() {
    currentPlayerIndex = (currentPlayerIndex + 1) % players.length;
    // Update the instructions with the new turn and current word
    instructions.innerText += "\nNow it is " + players[currentPlayerIndex] + "'s turn. Current word: " + sWord + ".";
  }

  function split() {
    // Create the second input field
    userInput2 = document.createElement("input");
    userInput2.type = "text";
    userInput2.id = "discardInput";
    userInput2.placeholder = "Discard word";
    document.body.appendChild(userInput2);

    // Update the placeholder for the first input field
    let userInput = document.querySelector("#userInput");
    userInput.placeholder = "Keep word";

    // Update Submit button behavior
    button1.onclick = splitvalCheck;

    // Change the split button to allow "unsplit"
    splitButton.innerText = "Unsplit";
    splitButton.onclick = unSplit;
  }

  function unSplit() {
    // Remove the second input field if it exists
    if (userInput2) {
      document.body.removeChild(userInput2);
      userInput2 = null;
    }
    // Reset Submit button behavior
    button1.onclick = valCheck;
    // Restore split button functionality
    splitButton.innerText = "Split";
    splitButton.onclick = split;
  }

  function splitvalCheck() {
    let userInputD = document.querySelector("#discardInput");
    let userInputK = document.querySelector("#userInput");

    let discardWord = userInputD.value.toLowerCase();
    let keepWord = userInputK.value.toLowerCase();

    if (
      discardWord.length + keepWord.length === sWord.length &&
      (discardWord.length === Math.ceil(sWord.length / 2) || discardWord.length === Math.floor(sWord.length / 2)) &&
      (keepWord.length === Math.ceil(sWord.length / 2) || keepWord.length === Math.floor(sWord.length / 2))
    ) {
      if (!usedWords.includes(keepWord)) {
        let isValid = true;
        let inputC = discardWord + keepWord;
        for (let i = 0; i < sWord.length; i++) {
          if (inputC.includes(sWord[i])) {
            inputC = inputC.replace(sWord[i], "");
          } else {
            isValid = false;
            break;
          }
        }
        if (isValid) {
          instructions.innerText = players[currentPlayerIndex] + " made a valid split. New word is: " + keepWord + ".";
          sWord = keepWord; // Update current word
          usedWords.push(sWord);
          unSplit(); // Remove the extra input field and reset buttons
          nextTurn(); // Change turn to the next player
        } else {
          instructions.innerText = "Invalid split. " + players[currentPlayerIndex] + ", your words did not include all the letters. Current word: " + sWord + ".";
        }
      } else {
        instructions.innerText = "This word has already been used. " + players[currentPlayerIndex] + ", try a different word. Current word: " + sWord + ".";
      }
    } else {
      instructions.innerText = "Invalid split lengths. " + players[currentPlayerIndex] + ", try again. Current word: " + sWord + ".";
    }
    // Clear the input boxes
    userInputD.value = "";
    userInputK.value = "";
  }

  function valCheck() {
    let userInput = document.querySelector("#userInput");
    let newWord = userInput.value.toLowerCase();

    if (userInput.value.length === sWord.length + 1) {
      if (!usedWords.includes(newWord)) {
        let isValid = true;
        for (let i = 0; i < sWord.length; i++) {
          if (newWord.includes(sWord[i])) {
            newWord = newWord.replace(sWord[i], "");
          } else {
            isValid = false;
            break;
          }
        }
        if (isValid) {
          instructions.innerText = players[currentPlayerIndex] + " made a valid move. New word is: " + userInput.value + ".";
          sWord = userInput.value;
          usedWords.push(sWord);
          nextTurn(); // Change turn to the next player
        } else {
          instructions.innerText = "Invalid word. " + players[currentPlayerIndex] + ", your word does not contain all the letters. Current word: " + sWord + ".";
        }
      } else {
        instructions.innerText = "This word has already been used. " + players[currentPlayerIndex] + ", try again. Current word: " + sWord + ".";
      }
    } else {
      instructions.innerText = "Incorrect length. " + players[currentPlayerIndex] + ", try again. Current word: " + sWord + ".";
    }
    // Clear the input box
    userInput.value = "";
  }
</script>
</body>
</html>

    
    
     
      
      
     
     
                     
      


   
               
    





    
   
       
              
    
           
    
